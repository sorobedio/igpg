...
from zoodatasets.transf_datasets import ZooDataset
...
class ZooDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        ...
        if stage == "fit":
            self.trainset = ZooDataset(root=self.zoo_root, split=self.zoo_split, transform=self.transform,
                                       scale=self.scale, tgt=self.tgt, exd=self.exd, length=self.length,
                                       to_image=self.to_image, n_tok=self.n_tok, input_size=self.resolution,
                                       in_ch=self.in_channel
                                       )


            self.valset = ZooDataset(root=self.zoo_root, split=self.zoo_split, transform=self.transform,
                                       scale=self.scale, tgt=self.tgt, exd=self.exd, length=self.length,
                                       to_image=self.to_image, n_tok=self.n_tok, input_size=self.resolution,
                                       in_ch=self.in_channel
                                       )
