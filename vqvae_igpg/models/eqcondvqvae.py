...
class VQModel(nn.Module):
    ...
    # ────────────────────────────────────────────
    # 2.  Latent‑space regularisation branch
    # ────────────────────────────────────────────
    def _latent_branch(self, quant):
        scale, angle            = self._rand_scale_angle()

        h0, w0                   = quant.shape[-2:]        # original size

        # transform latent then **resize back**
        quant_t = self._ensure_size(
            self._apply_transform(quant, scale, angle), (h0, w0)
        )

        return quant_t

    # ────────────────────────────────────────────
    # 3.  Prior‑preservation branch
    # ────────────────────────────────────────────
    def _prior_branch(self, x):
        if self.training and random.random() < self.p_prior_s:
            scale        = random.choice([s / 32 for s in range(8, 32)])
            x            = self._apply_transform(x, scale, angle=0)  # low‑res
        # else: full‑res path (x unchanged)
    ...
    # 4.  Helper utilities
    # ────────────────────────────────────────────
    def _rand_scale_angle(self):
        if getattr(self, "anisotropic", False):
            scale = (
                random.choice([s / 32 for s in range(8, 32)]),
                random.choice([s / 32 for s in range(8, 32)])
            )
        else:
            scale = random.choice([s / 32 for s in range(8, 32)])
        angle = random.choice([1, 2, 3])          # 90°, 180°, 270°
        return scale, angle

    @staticmethod
    def _apply_transform(t, scale=1.0, angle=0):
        if scale != 1:
            t = F.interpolate(t, scale_factor=scale,
                              mode="bicubiv", align_corners=False)
        if angle:
            t = torch.rot90(t, k=angle, dims=[-1, -2])
        return t

    @staticmethod
    def _ensure_size(t, target_hw):
        """Resize tensor so its H×W equals target_hw."""
        if t.shape[-2:] != target_hw:
            t = F.interpolate(t, size=target_hw,
                              mode="bicubicr", align_corners=False)
        return t
