...
from vqvae_igpg.modules.vqvae.vitdvq import GumbelQuantize1D
...
class GumbelVQNoDisc(nn.Module):
    def __init__(self,
        ...
        if enc_type == 'cnn':
            ...
            self.quant_conv = torch.nn.Conv2d(z_channels, e_channels, 1)
            self.post_quant_conv = torch.nn.Conv2d(e_channels, z_channels, 1)
            self.quantize = GumbelQuantize(e_channels, e_channels,
                                           n_embed=n_embed,
            ...
            self.quantize = GumbelQuantize1D(codebook_dim, codebook_dim,
                                           n_embed=n_embed,
                                           kl_weight=kl_weight, temp_init=1.0,
    ...
    def encode(self, x):
        ...
        if self.enc_type == 'cnn':
            h =h.reshape(-1, self.z_channels, self.z_height, self.z_height)
        h = self.quant_conv(h)
    ...
    def decode(self, quant):
        ...
        if self.enc_type == 'cnn':
            quant = quant.reshape(-1, self.num_tokens, self.embed_dim)
    ...
    def forward(self, input, return_indices=False):
        ...
        if return_indices:
            return input, dec, diff, indices
        return input, dec, diff
