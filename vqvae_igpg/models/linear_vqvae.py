...
from vqvae_igpg.modules.linear_modules import Encoder1DLinear as Encoder
from vqvae_igpg.modules.linear_modules import Decoder1DLinear as Decoder
from vqvae_igpg.modules.vqvae.linearquantize import VectorQuantizer2_1D as VectorQuantizer
from vqvae_igpg.modules.vqvae.linearquantize import GumbelQuantize1D as GumbelQuantize
from vqvae_igpg.modules.vqvae.linearquantize import EMAVectorQuantizer1D as EMAVectorQuantizer
...
class VQModel(pl.LightningModule):
    def __init__(self,
        ...
        #nn.Linear(num_hiddens, n_embed)
        # self.post_quant_conv = torch.nn.Conv2d(embed_dim, ddconfig["z_channels"], 1)

        # 1 × 1 convolutions become Conv1d when the data is 1‑D.
        self.quant_conv = torch.nn.Conv1d(ddconfig["z_channels"], embed_dim, kernel_size=1)
        self.post_quant_conv = torch.nn.Conv1d(embed_dim, ddconfig["z_channels"], kernel_size=1)
...
class GumbelVQNoDisc(VQModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("temperature", self.quantize.temperature, prog_bar=False, logger=True, on_step=True,
                 on_epoch=True)
        # self.log({"z_mean": quant.mean().item(), "z_std": quant.std().item()})
