...
# ------------------------------------------------------------
#  helpers.py  (or drop at the top of your vqvae.py)
# ------------------------------------------------------------
...
class AugCfg:
    # ‑‑ input‑space aug ‑‑
    p_input_scale: float = 0.20  # case‑2
    p_input_rotate: float = 0.20  # case‑3
    min_scale_in: float = 8 / 32  # isotropic, [8/32 … 31/32]
    max_scale_in: float = 31 / 32

    # ‑‑ latent‑space aug ‑‑
    p_latent_rot: float = 0.30  # case‑1  (rotation)
    p_latent_scale: float = 0.30  # case‑1  (scaling)
    min_scale_lat: float = 0.5  # isotropic, continuous
    max_scale_lat: float = 1.5


# ------------------------------------------------------------
#  patch these methods into your VQModel class
# ------------------------------------------------------------
@staticmethod
def _rand_rotate( x: torch.Tensor, p: float) -> Tuple[torch.Tensor, Optional[int]]:
    """With probability p rotate by 0,90,180,270°; return k so caller can un‑rotate later if desired."""
    if torch.rand(1).item() >= p:
        return x, None
    k = torch.randint(0, 4, ()).item()
    return torch.rot90(x, k, dims=(-2, -1)), k  # (B,C,H,W)


@staticmethod
def _rand_isotropic_scale(
    ...
    if torch.rand(1).item() >= p:
        return x
    r = torch.empty(1).uniform_(r_min, r_max).item()
    H, W = x.shape[-2:]
    x = F.interpolate(x, scale_factor=r, mode=mode, align_corners=False)
    # back to original canvas (centre‑crop or pad replicate)
...
class VQModel(nn.Module):
    ...
    def _augment_input(self, x: torch.Tensor, cfg: AugCfg) -> torch.Tensor:
        # ‑‑ case‑2 : down/up scale (blurs) ‑‑
        x = self._rand_isotropic_scale(
            x, cfg.p_input_scale, cfg.min_scale_in, cfg.max_scale_in, mode="bicubic"
        )
        # ‑‑ case‑3 : rotate whole image ‑‑
        x, _ = self._rand_rotate(x, cfg.p_input_rotate)
        return x

    def _augment_latent(self, z: torch.Tensor, cfg: AugCfg) -> torch.Tensor:
        # ‑‑ case‑1a : rotate latent grid ‑‑
        z, _ = self._rand_rotate(z, cfg.p_latent_rot)
        # ‑‑ case‑1b : isotropic resize of latent grid ‑‑
        z = self._rand_isotropic_scale(
            z, cfg.p_latent_scale, cfg.min_scale_lat, cfg.max_scale_lat, mode="bicubic"
        )
        return z
    ...
    def forward(
        ...
        """
        Implemented cases
          1.  Random rotate / scale **latent grid**
          2.  Random isotropic down‑scale *input*  (blur)
          3.  Random 90° rotate *input*
          4.  (codebook untouched → nothing to do)
        """
        if self.training:
            x = self._augment_input(x, aug_cfg)  # cases 2 + 3

        # ---------- encode + quantise ----------
        quant, diff, (_, _, ind) = self.encode(x)  # (B,Cz,h,w)

        if self.training:
            quant = self._augment_latent(quant, aug_cfg)  # case‑1
    ...
    #     c = self.get_learned_conditioning(**cond)
    #     """
    #       Training: randomly chooses latent‑reg or prior‑preservation branch.
    #       Eval:     behaves like a plain VQ‑VAE (because self.training == False).
    #       """
    #     if self.training and random.random() < self.p_prior:
    #         quant, diff, (_, _, ind) = self.encode(inputs)
    #         dec, diff, ind = self._latent_branch(quant)
    #     else:
    #         inputs = self._prior_branch(inputs)
    #         quant, diff, (_, _, ind) = self.encode(inputs)
    #
    #     dec = self.decode(quant)
    #
    ...
    #             # print(inputs.shape)
    #             quant, diff, (_, _, ind) = self.encode(inputs)
    #             quant, scale, angle = self.process_latent(quant)
    #             inputs = self.interpolate(inputs, scale)
    #             # print(inputs.shape)
    #             inputs = self.rotate(inputs, angle)
    #             # print(inputs.shape)
    #
    #     else:
    #         mode = "weight-space"
    #         inputs = self.process_image(inputs)
    #         # print(inputs.shape)
    #         quant, diff, (_, _, ind) = self.encode(inputs)
    #     # quant, diff,  (_,_,ind)= self.encode(inputs)
    #     # quant, diff,  (_,_,ind) = self.latent_to_code(z)
    #     # print('=========================decoding==================')
    #     print(quant.shape)
    #
    #     dec = self.decode(quant, c)
