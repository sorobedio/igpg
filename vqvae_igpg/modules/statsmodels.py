...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 in_dim=2864, fdim=8192, my_channels=24, stat_len=3,
                 inreshape=True,
                 **ignore_kwargs):
        super().__init__()
        self.ch = ch
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
                 ...
                 in_dim=2864, fdim=8192, my_channels=24,
                 inreshape=True,
                 **ignorekwargs):
        super().__init__()
        self.in_dim = in_dim
    ...
    def forward(self, z):
        ...
        h = nonlinearity(h)
        h = self.conv_out(h)
        # h = h.reshape(-1, self.out_ch, self.resolution**2)
        h = h.reshape(-1, self.fch, self.fdim)
        h = self.fc_out(h)
        # print(h.shape)
        if self.inreshape:
            h= h.reshape(-1, self.fch*self.in_dim)
        return h
