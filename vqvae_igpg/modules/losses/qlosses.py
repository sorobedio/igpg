...
class VQLoss(nn.Module):
    def __init__(self, codebook_weight=1.0, loss_type='l1'):
        super().__init__()
        self.codebook_weight = codebook_weight
        self.loss_type=loss_type
        self.pad_value = 0
        ...
        # self.pixel_weight = pixelloss_weight
    def forward(self, codebook_loss, inputs, reconstructions, split="train"):
        rec_loss = F.cross_entropy(reconstructions.transpose(1, 2), (inputs + 1).long(),
                                   reduction='mean')
        nll_loss = rec_loss
        ...
        log = {"{}/total_loss".format(split): loss.clone().detach(),
               "{}/quant_loss".format(split): codebook_loss.detach().mean(),
               "{}/rec_loss".format(split): rec_loss.detach()*1000,
               }
        return loss, log
