...
class RandomPermuteTransform:
    def __init__(self, permute_rate=0.3):
        """
        Args:
            permute_rate (float): Probability of permuting the elements in the input tensor.
        """
    ...
    def __call__(self, x):
        ...
        Returns:
            torch.Tensor: Transformed tensor with permuted elements (if applicable)
        """
        if torch.rand(1).item() < self.permute_rate:
            idx = torch.randperm(x.numel())  # Random ordering of indices
            return x[idx]  # Return a new tensor with permuted elements
        return x
...
class ZooDataset(Dataset):
    ...
    def __init__(self, root='modelzoos', dataset="Llama-3.2-3B-Instruct", split='train', topk=None,
        ...
        self.transform = RandomPermuteTransform(permute_rate=0.3)
